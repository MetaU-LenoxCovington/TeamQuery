...
export class PermissionService {
  ...
  async getAccessibleDocumentIds(
    ...
    if (
      permissions.role === MembershipRole.MANAGER ||
      permissions.role === MembershipRole.ADMIN
    ) {
  ...
  async canAccessDocument(
    ...
    switch (document.accessLevel) {
      ...
      case AccessLevel.GROUP:
        ...
        return permissions.groups.some(
          (group: any) => group.id === document.groupId
        );
      ...
      case AccessLevel.RESTRICTED:
        if (
          document.restrictedToUsers.includes(userId) ||
          permissions.isAdmin
        ) {
          return true;
        } else {
